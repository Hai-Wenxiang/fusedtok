...
- **(honest)** marks rows where fusedtok loses: attention_prefill vs
  ...
  parallel sort (0.16-0.24x for the singles; the batched flat rows
  fall to 0.05-0.06x - see the batched table), the wide-nucleus
  sample_minp row (0.33-0.39x - one widening retry plus a 32-64k
  sort; the torch boolean-mask composite never sorts) and the
  sample_nsigma rows (0.08-0.13x - the sigma cutoff keeps the top
  ...
  design scope (regimes these ops never aimed to win), not
  measurement noise - the
  [topic pages](usage.md) explain each one.
- The **batched-sampler rows** (`b=8`) reference torch's native 2-D
  draw - softmax plus `multinomial` over the whole `[8, vocab]`
  tensor (with top-k or a boolean mask where the row label says so) -
  not the per-row composites the single-row rows use; the
  batched-vs-per-row-loop comparison (4-6x wall on submission-bound
  hosts) lives in the README prose because both table columns must be
  library calls. The `decode_step_batched` row's reference
  additionally applies the per-row gather-penalty before the draw.
...
- **argmax** rows are event-timed over a host-synchronized call and
  swing on WDDM (0.67-0.98x across the 3060's shipped rounds); wall-clock probes with the
