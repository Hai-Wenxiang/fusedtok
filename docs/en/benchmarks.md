...
- **(honest)** marks rows where fusedtok loses: attention_prefill vs
  ...
  parallel sort (0.16-0.35x for the singles; the batched flat rows
  fall to 0.05-0.07x - see the batched table), the wide-nucleus
...
- **argmax** rows are event-timed over a host-synchronized call and
  swing on WDDM (0.67-1.21x across the 1.8.0 rounds); wall-clock probes with the
