...
- **(honest)** marks rows where fusedtok loses: attention_prefill vs
  ...
  (0.40-0.58x), and flat-distribution sample_topp vs torch's fully
  parallel sort (0.10-0.17x). These are design-scope statements, not
  measurement noise - the [topic pages](usage.md) explain each one.
