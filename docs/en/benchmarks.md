...
- **(honest)** marks rows where fusedtok loses: attention_prefill vs
  ...
  parallel sort (0.16-0.25x for the singles; the batched flat rows
  fall to 0.05-0.06x - see the batched table), the wide-nucleus
  sample_minp row (0.34-0.40x - one widening retry plus a 32-64k
...
- **argmax** rows are event-timed over a host-synchronized call and
  swing on WDDM (0.74-1.48x across the 2.1.0 rounds); wall-clock probes with the
