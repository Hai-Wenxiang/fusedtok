...
| `TypeError` | dtype / device-family problems | float64 input on the zero-copy path, CPU tensor where the primary input is CUDA, mixed dtypes between q/k/v, operands on mismatched devices (each operator documents which operand may live where) |
