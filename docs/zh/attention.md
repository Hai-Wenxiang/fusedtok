...
fusedtok 提供四个注意力入口：解码步主力 `attention_decode`、
分页 cache 变体 `attention_decode_paged`（v1.2，配套写侧
`kv_append_paged`）、以及 prefill 便捷路径 `attention_prefill`。
...
GQA 映射、零行约定、dtype 矩阵与连续版完全一致；切分管线和
workspace 也是共用的。分页版特有的规则：
...
P=16）：连续版的 **1.06–1.07 倍**，且切片调度一致时输出逐位
相同。
...
- **原地写**（返回 `None`）。块表归调度器管：本算子只往已映射
  的块里写数据，绝不动表项。
- 主机路径要求 float32 且 C 连续的池——dtype/布局转换会看到一个
  副本、静默丢掉写入，所以直接 `TypeError` 拒绝而不是默默出错。
  torch 路径支持 f32/bf16/fp16 全部存储组合。
...
- f32 decode 在长 cache 上达到或超过 SDPA 的有效带宽
  （README 表里 3060 @T=16384 最高 8.79x——参考实现在那边要付
  头展开或小 query 低效的代价）。
- bf16/fp16 cache 把字节减半。batch 为 1 时 kernel 受延迟限制，
  绝对收益有限，batch 越大收益越大。
- 分页间接寻址比连续版多付 1.06–1.07 倍。
