...
分数全程不物化（不会物化成显存里的中间张量）；q/K/V 各恰好读一遍。
...
- 主机侧来源的 `lens` 取值在 `[0, T)` 内校验；设备上的张量直接信任
  （标准零拷贝信任边界）。
- 性能（基准表）：3060 上 14.4 µs vs torch 高级索引 95.5 µs
  （3.64x）、5060 Ti 上 9.4 µs vs 21.3 µs（2.26x）——算子本身很小、
...
- f32 decode 在长 cache 上达到或超过 SDPA 的有效带宽
  （README 表里 3060 @T=16384 最高 8.83x——参考实现需要额外做头
  展开，且在小 query 下效率偏低）。
