...
- 实现说明：exp 列本就按行最大归一（`exps[0] == 1.0`），核就是
  ...
  一样自适应跳变：失败的窗口留下累计质量，配上一次性的全局总量
  即可推出必定盖住核的下界（`w >= W + (T − C) / min_p`），宽核
  从此跳过 x8 阶梯的中转站（宽核行约快 30%，token 逐位不变）。
...
一次调用采样整个 `[行数, 词表]` 批，每行返回一枚 token（int64；
torch 输入回 CPU torch 张量，numpy 输入回 numpy 数组——扩窗循环
天然要回读主机，因此与单行版一样**不可** CUDA 图捕获）。

- `logits` 为二维、连续、float32。`seeds` 每行一个非负整数；
  默认（`None`）取 `0..行数-1`，相同内容的行也能各自独立抽签。
- 每行跑的就是单行管线**原样**——同样的 kernel、同样的累加顺序、
  逐行一致（含扩窗循环：各行按自己的核宽度完成，先完成的行被
  跳过，宽核的行继续加宽重试）。
- 行按 32 一组分块处理，超大批次也只在有界的 workspace 内流过。
- 批处理买到的东西：逐行的 Python/启动开销坍缩成一次——B=8 在
  提交受限主机上墙钟时间是逐行循环的 1/4 到 1/6（3060 上
  [8, 131072]：topp 1340 -> 274 µs、minp 1399 -> 237 µs），尖峰
  logits 下落到 torch 原生批量 multinomial 的档位
  （`sample_topk_batched` 直接领先：2.33x / 1.25x）。平坦最坏
  情况则保持与单行版相同的诚实劣势（0.05-0.06x）。
- `decode_step` 暂无批量版（每行的惩罚位图要多一个维度；见 1.5
  路线图）。
...
工作时暴露，属性自 1.2 的单行 API 就存在）：全局 softmax 总量靠
逐 block 的浮点原子加累加，到达序在进程间可能不同——恰好落在
CDF 边界上的抽签在进程重启后可能抽到相邻 token（131k 下约 8 行
里观察到 1 次；实际上几乎不会遇到）。批量采样器每行用的是同一
模式，因此某行与它的单独调用也可能差这一个边界 token——parity
测试按"精确或相邻排名"钉住。
...
——基准表里如实标着 0.17–0.25x。v1.2 用三个不破坏契约的改动把
...
了一项同样保持 token 逐位不变的优化：walk 1 按批边界把前缀和记进
共享内存，walk 2 用二分查找定位目标所在的批、从那里续走（续走的
前缀与从头走逐位相同——同样的加数按同样的顺序），平坦最坏情况再
压到约 1/1.6。
