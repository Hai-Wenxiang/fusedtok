...
必须给核**排序**（选择管线的职责），而接近
均匀的 logits 下核约占九成词表，所以这个场景确实更慢（批量 0.05-0.06x、单行 0.16-0.25x）。真实解码的 logits 是尖峰状的，那
才是采样器与原生 multinomial 同档甚至更快的场景。细节见
[采样——平坦分布](sampling.md#平坦分布不回避的最坏情况)。
...
根据回读结果重新启动 kernel，无论返回值落在主机还是设备，都进不了图；
...
event 而不是墙上时钟，并预期 argmax 这类微小算子的数字会明显波动。
...
- **分页 cache / 块表（paged cache / block table）**——token
  分散存放在由定长块组成的池中、按序列用块表索引的 cache 排布
...
- **eta 截断（eta-cutoff）**——阈值由分布自身熵决定的采样截断：
  ...
  单位 nat）——自信（尖峰）分布截断很重，平坦分布几乎不截
  （阈值沉向零）。
- **top-a**——阈值跟随**平方峰值**的采样截断：保留所有
  `p_i >= top_a × p_max²` 的 token——自信分布保住紧凑头部，接近
  均匀的分布几乎保留整个词表（阈值的下降比峰值本身更快）。
- **尾部自由采样（TFS）**——按排序 CDF 二阶导数截断：保留概率曲线
  仍在"弯曲"（归一化 |d2| ≥ 1-z）的前缀，剪掉实质平坦的尾部。
- **top-nσ**——阈值跟随**离散度**的采样截断：保留所有缩放后
  logit 不低于 `均值 − nsigma × 标准差` 的 token（矩对整行取）——
  裁掉的是分布自身的长尾，平坦行全保留。
