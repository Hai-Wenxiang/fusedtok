...
- **（落败）** 标注的是 fusedtok 输掉的行：attention_prefill 对
  ...
  （单行 0.16-0.24x；批量平坦行再低一档，0.05-0.06x，见批量表）、
  宽核的 sample_minp 行（0.33-0.39x——一次扩窗重试加一次
  32-64k 排序；torch 的布尔掩码组合式从不排序），以及
  sample_nsigma 行（0.08-0.13x——σ 阈值会保留尖峰行约前 94% 的
...
- **argmax** 行是对含主机同步调用的事件计时，在 WDDM 上摆动大
  （3060 随库发布的三轮计时在 0.67-0.98x 间摆动）；把同步排除在计时循环之外的墙钟实测为
