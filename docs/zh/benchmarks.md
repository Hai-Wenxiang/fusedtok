...
- **带宽列**（GB/s）只统计该算子必须搬运的字节（如 softmax 2
  ...
  数 ×2 计算。
- **环境注记（2.4.0 轮）**：3060 主机本轮在启动延迟敏感的行上比 2.3.0
  轮整体慢约 1.6 倍（torch 参考同步变慢——sample_topp b=8 的比值只从
  1.43x 挪到 1.47x；带宽型行不受影响）。会话中无法做重启级的环境复位，
  因此本轮 3060 的绝对值内部自洽但不与 2.3.0 可比；比值不受影响。
