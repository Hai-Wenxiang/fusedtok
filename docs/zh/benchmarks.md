...
- 采样行测量**固定的 logits**：bench 给 torch 的 RNG 播种，每次
  ...
  尖峰、平坦行用接近均匀的 logits，两种分布形态的抽样都完全确定，名副其实。
...
- **（落败）** 标注的是 fusedtok 输掉的行：attention_prefill 对
  ...
  （单行 0.16-0.25x；批量平坦行再低一档，0.05-0.06x，见批量表）、
  宽核的 sample_minp 行（0.34-0.40x——一次加宽重试加一次
  32-64k 排序；torch 的布尔掩码组合式从不排序），以及
  sample_nsigma 行（0.08-0.11x——σ 门槛会保留尖峰行约前 94% 的
  词表，接近全词表采样走完整加宽阶梯；nsigma 剪的是长尾，收益
  主张在输出质量而非速度）。这些是设计范围的声明，不是测量
  噪声——各[主题页](usage.md)有解释。
...
- **argmax** 行是对含主机同步调用的事件计时，在 WDDM 上摆动大
  （2.1.0 三轮跨轮 0.74-1.48x）；把同步排除在计时循环之外的墙钟实测为
