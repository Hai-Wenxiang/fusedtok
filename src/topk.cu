...
__global__ void radix_topk_kernel(const float* __restrict__ x,
                                  float* __restrict__ vals,
                                  long long* __restrict__ idxs,
                                  int* __restrict__ count_out,
                                  int n, int k, int m, float p_stop,
                                  unsigned long long* __restrict__ ws) {
    const int grid_blocks = gridDim.x;
    cg::grid_group grid = cg::this_grid();

    unsigned long long* hist = ws;                 // 256 bins
    unsigned long long* emit_cnt = ws + 256;
    unsigned long long* tie_cnt = ws + 257;
    unsigned long long* g_prefix = ws + 258;       // refinement state in GLOBAL
    unsigned long long* g_remaining = ws + 259;    // memory: every block must
    unsigned long long* keys = ws + 260;           // see the same prefix

    if (blockIdx.x == 0) {
        for (int i = threadIdx.x; i < 260; i += blockDim.x) ws[i] = 0ULL;
        if (threadIdx.x == 0) *g_remaining = (unsigned long long)k;
        for (long long i = k + threadIdx.x; i < m; i += blockDim.x)
    ...
    const unsigned long long k_min = *g_prefix;    // k-th largest key
    ...
    if (m <= 2048) {
        ...
        if (blockIdx.x == 0) {
            ...
            __syncthreads();
            for (int i = threadIdx.x; i < k; i += blockDim.x) keys[i] = sk[i];
        }
        grid.sync();
    } else {
    ...
    if (blockIdx.x == 0) {
        ...
        if (p_stop > 0.0f && threadIdx.x == 0) {
            // nucleus: cumulative mass over descending values until >= p
            float cum = 0.0f;
            int count = 0;
            for (int i = 0; i < k; ++i) {
...
void launch_radix_topk(const float* x, float* vals, long long* idxs,
    ...
    int n_ = n, k_ = k, m_ = m;
    float p_ = p_stop;
    unsigned long long* ws_ = ws;
    void* args[] = {&x_, &vals_, &idxs_, &count_, &n_, &k_, &m_, &p_, &ws_};
...
void select_common(const float* x, float* vals, long long* idxs,
                   int* count_out, int n, int k, float p_stop) {
    if (coop_supported()) {
        launch_radix_topk(x, vals, idxs, count_out, n, k, p_stop);
    } else {
