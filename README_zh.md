...
等，支持**torch 张量零拷贝**：对比 PyTorch SDPA 最高 **8.9 倍加速**
...
共 34 个算子与辅助函数（1.x 系列冻结的 `fusedtok.__all__`，见下方"API
...
|---|---|---:|---:|---:|
| attention_decode（GQA） | T=16384, D=128 | 873 µs | 7783 µs（SDPA） | **8.91x** |
| attention_decode_paged（GQA） | T=16384, D=128, P=16 | 984 µs | 7830 µs（SDPA） | **7.96x** |
| RoPE NeoX (q+k) | [8192×4096] | 1662 µs | 10196 µs | **6.13x** |
| kv_append（连续 cache 写入） | B=8, T=4096 | 17 µs | 89 µs（高级索引） | **5.31x** |
| RMSNorm（含残差） | [4096×4096] | 627 µs | 2087 µs | **3.33x** |
| sample_topp p=0.9（峰值分布） | [131072] | 162 µs | 511 µs（排序+掩码+multinomial） | **3.16x** |
| attention_decode bf16 | T=16384, D=128 | 872 µs | 1828 µs（SDPA bf16） | **2.10x** |
| sample_topk k=50 | [131072] | 148 µs | 302 µs（topk+multinomial） | **2.04x** |
| SwiGLU | [4096×4096] | 620 µs | 1036 µs | **1.67x** |
| top-k (k=50) | [131072] | 82 µs | 129 µs | **1.57x** |
| LayerNorm | [4096×4096] | 452 µs | 624 µs | **1.38x** |
| sample_minp p=0.05（峰值分布） | [131072] | 170 µs | 211 µs（掩码+multinomial） | **1.24x** |
| top-k（k=4096，中段 k） | [131072] | 119 µs | 139 µs | **1.17x** |
| Softmax | [4096×4096] | 419 µs | 436 µs | **1.04x** |
| SiLU / GeLU / add | [4096×4096] | ~416-616 µs | ~417-620 µs | ~1.0x |
| argmax | [131072] | 53 µs | 42 µs | 0.80x（事件计时在 WDDM 上抖动大；wall 探针 1.12x，见下） |
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 3591 µs（38.3 TOPS） | 2066 µs（cuBLASLt + 广播） | 0.58x（诚实） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 9765 µs（37.8 TOPS） | 4477 µs（cuBLASLt） | 0.46x（诚实） |
| attention_prefill（因果） | S=1024, D=128 | 5931 µs | 2645 µs（SDPA flash） | 0.45x（诚实） |
| sample_topp p=0.9（平坦最坏） | [131072] | 1447 µs | 536 µs | 0.37x（诚实，见下） |
| sample_minp p=0.05（宽核） | [131072] | 679 µs | 217 µs | 0.32x（诚实：两次加宽重试加一次 64k 排序；torch 的布尔掩码组合式不用排序——min-p 的胜出场景见上方峰值行） |
...
| 算子 | 形状 | fusedtok | PyTorch 参考 | 加速比 |
|---|---|---:|---:|---:|
| RoPE NeoX (q+k) | [8192×4096] | 1385 µs | 8372 µs | **6.05x** |
...
| sample_topp p=0.9（峰值分布） | [131072] | 62 µs | 154 µs（排序+掩码+multinomial） | **2.47x** |
| sample_topk k=50 | [131072] | 45 µs | 92 µs（topk+multinomial） | **2.03x** |
| SwiGLU | [4096×4096] | 504 µs | 859 µs | **1.70x** |
| top-k (k=50) | [131072] | 27 µs | 41 µs（CUB） | **1.51x** |
| sample_minp p=0.05（峰值分布） | [131072] | 62 µs | 72 µs（掩码+multinomial） | **1.17x** |
| top-k（k=4096，中段 k） | [131072] | 50 µs | 54 µs（CUB） | 1.09x |
| LayerNorm / Softmax | [4096×4096] | ~346 µs | ~345-351 µs | ~1.0x |
| argmax | [131072] | 17 µs | 14 µs | 0.80x（事件计时有噪声；wall 探针 0.96x） |
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 2078 µs（66.1 TOPS） | 1142 µs（cuBLASLt + 广播） | 0.55x（诚实） |
| attention_prefill（因果） | S=1024, D=128 | 3300 µs | 1421 µs（SDPA flash） | 0.43x（诚实） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 5481 µs（67.4 TOPS） | 2186 µs（cuBLASLt） | 0.40x（诚实） |
| kv_append（连续 cache 写入） | B=8, T=4096 | 9 µs | 21 µs（高级索引） | 2.21x |
| sample_topp p=0.9（平坦最坏） | [131072] | 984 µs | 164 µs | 0.17x（诚实，见下） |
| sample_minp p=0.05（宽核） | [131072] | 366 µs | 72 µs | 0.20x（诚实：同 3060 行的宽核说明） |
...
胜过 eager 组合式（sample_topp 峰值：3.16x / 2.47x；sample_topk：
2.04x / 2.03x；峰值行的组合参考本身在 WDDM 上逐轮波动约 15%，
逐轮数值在 JSON 里）；平坦分布下 sample_topp 诚实为 0.17-0.37x —— 此时核覆盖
...
组合式 1.24x / 1.17x），宽核行诚实落后（0.2-0.32x：两次加宽重试加一次 64k
...
（比手写高级索引快 5.31x / 2.21x；算子本身很小、受启动开销限制——
量的是每步解码的固定成本）。
