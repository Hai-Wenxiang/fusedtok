...
| ✅ | attention_decode_paged | 1.2 主打特性：同样的解码注意力跑在 **vLLM 式块池 kv-cache** `[Nb, Hkv, P, D]` 上，经每序列块表间接寻址——cache 内存零碎片；任意合法表均可，f32/bf16/fp16 存储，约为连续版 1.09-1.14x 开销（相对预展开头参考为 SDPA 的 7.95x / 4.28x） |
...
| kv_append（连续 cache 写入） | B=8, T=4096 | 14 µs | 49 µs（高级索引） | ** ** 3.55x**** |
| sample_topp p=0.9（峰值分布） | [131072] | 142 µs | 411 µs（排序+掩码+multinomial） | ** ** 2.89x**** |
| RMSNorm（含残差） | [4096×4096] | 629 µs | 2125 µs | ** ** 3.38x**** |
...
| sample_minp p=0.05（峰值分布） | [131072] | 151 µs | 199 µs（掩码+multinomial） | ** ** 1.32x**** |
| sample_topa a=0.2（峰值分布） | [131072] | 182 µs | 189 µs（掩码+multinomial） | ** ** 1.04x**** |
| top-k（k=4096，中段 k） | [131072] | 116 µs | 112 µs | 0.97x（落败） |
| Softmax | [4096×4096] | 421 µs | 441 µs | ** ** 1.05x**** |
| SiLU / GeLU / add | [4096×4096] | ~418-626 µs | ~420-626 µs | ~1.0x |
| argmax | [131072] | 55 µs | 46 µs | 0.85x（事件计时在 WDDM 上抖动大；本轮跨轮 0.67-1.21x，见下） |
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 3672 µs（37.4 TOPS） | 2116 µs（cuBLASLt + 广播） | 0.58x（落败） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 9862 µs（37.5 TOPS） | 4665 µs（cuBLASLt） | 0.47x（落败） |
| attention_prefill（因果） | S=1024, D=128 | 5902 µs | 2647 µs（SDPA flash） | 0.45x（落败） |
| sample_minp p=0.05（宽核） | [131072] | 531 µs | 205 µs | 0.39x（落败：一次加宽重试加一次 32-64k 排序；torch 的布尔掩码组合式不用排序——min-p 的胜出场景见上方峰值行） |
| sample_nsigma 1.5（峰值分布） | [131072] | 1693 µs | 215 µs（掩码+multinomial） | 0.13x（落败：σ 门槛会保留尖峰行约前 94% 的词表——nsigma 剪的是分布的低处长尾，尖峰行因此接近全词表采样、走完整加宽阶梯；论文主张 nsigma 的收益在输出质量而非速度） |
| sample_topp p=0.9（平坦最坏） | [131072] | 1393 µs | 482 µs | 0.35x（落败，见下） |
...
| sample_minp_batched p=0.05 | [8×131072] | 237 µs | 378 µs（掩码+multinomial） | ** ** 1.59x**** |
| sample_topa_batched a=0.2 | [8×131072] | 283 µs | 306 µs（掩码+multinomial） | ** ** 1.08x**** |
| sample_topp_batched p=0.9 | [8×131072] | 271 µs | 224 µs（multinomial） | 0.83x（参考侧 WDDM 波动，见下） |
| decode_step_batched（惩罚 1.3，~64 token 历史） | [8×131072] | 341 µs | 279 µs（惩罚+softmax+multinomial） | 0.82x（对比逐行循环墙上时钟 **5.2x**，见下方说明） |
| argmax_batched | [8×131072] | 33 µs | 24 µs（argmax） | 0.73x（对 torch 单 kernel 的逐行 argmax、事件计时），但对比逐行循环墙上时钟 **28x**（8 次提交 -> 1 次） |
| sample_nsigma_batched 1.5 | [8×131072] | 3863 µs | 335 µs（掩码+multinomial） | 0.09x（落败，同单行宽核说明） |
| sample_topp_batched（平坦最坏） | [8×131072] | 3767 µs | 266 µs | 0.07x（落败，同单行说明） |
...
| RMSNorm（含残差） | [4096×4096] | 504 µs | 1657 µs | ** ** 3.28x**** |
| sample_topp p=0.9（峰值分布） | [131072] | 62 µs | 156 µs（排序+掩码+multinomial） | ** ** 2.52x**** |
...
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 2078 µs（66.1 TOPS） | 1145 µs（cuBLASLt + 广播） | 0.55x（落败） |
| attention_prefill（因果） | S=1024, D=128 | 3300 µs | 1425 µs（SDPA flash） | 0.43x（落败） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 5484 µs（67.4 TOPS） | 2195 µs（cuBLASLt） | 0.40x（落败） |
...
5060 Ti 上 1.6ms -> 1.0ms——第一遍遍历记录前缀和、第二遍用二分定位
...
上摆动大（1.8.1 三轮跨轮 0.85-1.54x）；把同步排除在计时循环之外的墙钟实测为
...
- 1.2（已发布）：分页 kv-cache attention —— `attention_decode_paged` 跑在 vLLM 式块池 `[Nb, Hkv, P, D]` + 每序列块表上（连续版约 1.09-1.14x 开销，任意合法表均可）与 `kv_append_paged`（原地 cache 写入侧）；平坦分布采样最坏情况压到约 1/8.5（自适应扩窗跳变 + 全词表快路径 + 批量取数串行遍历，token 逐位不变）；argmax 减负（每次调用少一次提交少一次分配）
