...
共 57 个算子与辅助函数（其中 34 个在 1.0 冻结，其余经小版本加入，见下方"API
...
| ✅ | sample_topa / sample_topa_batched | 融合 top-a 采样（v1.8）：保留所有 p >= top_a × p_max² 的 token——与 min-p 同类的值阈值，但由平方峰值驱动阈值（平坦行几乎全保留）；截断值直接从既有总量推出，扩窗下界复用 min-p 的充分公式 |
| ✅ | sample_nsigma / sample_nsigma_batched | 融合 top-nσ 采样（v1.8，Shi 等 2024）：保留所有缩放后 logit 不低于 `均值 − nsigma × 标准差` 的 token——阈值跟随分布自身离散度；均值和方差只需一遍额外 pass（逐 block 先升为 double 再原子累加），方差的相减运算不会放大原子累加的到达顺序误差 |
...
| ✅ | sample_dry / sample_dry_batched | 融合 DRY 采样（v2.3）：序列级重复惩罚——惩罚的是"会延续重复序列的 token"，随后一次全词表 softmax 抽签（公开名称 53 -> 55） |
| ✅ | sample_mirostat / sample_mirostat_batched | 融合 Mirostat v2 采样（v2.5）：带调用方持有的 mu 界限的熵目标采样——绝对阈值核 p >= 2^-mu，融合状态更新 new_mu = mu - eta*(s - tau)；唯一返回元组的采样器（(token, new_mu)；公开名称 55 -> 57） |
| ✅ | argmax_batched | 整批 `[行数, 词表]` 的贪心下标，一次 launch 完成（v1.8）：零拷贝路径无主机回读、可 CUDA graph 捕获；受提交开销限制的主机上 B=8 墙钟耗时约为逐行循环的 1/28（快约 28 倍） |
...
| ✅ | logit_penalties | HF 三种惩罚一次调用完成（v1.6.1）：先按 CTRL 规则缩放重复 token，再做出现惩罚的减法偏移，再做按计数加权的频率偏移——每个去重 id 只罚一次、重复不叠加；CPU 与 GPU 逐位一致 |
...
| ✅ | attention_decode_paged | 1.2 主打特性：同样的解码注意力跑在 **vLLM 式块池 kv-cache** `[Nb, Hkv, P, D]` 上，经每序列块表间接寻址——cache 内存零碎片；任意合法表均可，f32/bf16/fp16 存储，约为连续版 1.09-1.14x 开销（相对预展开头参考为 SDPA 的 8.46x / 4.28x） |
| ✅ | kv_append_paged | 分页循环的 cache 写入侧：每序列一个新 token 的 k/v 行原地 scatter 到池中 `lens[b]` 位置（一个微型 kernel，f32/bf16/fp16） |
| ✅ | attention_prefill | 新序列 S 行注意力（因果 / 双向），f32/bf16/fp16 存储；v2.4 起半精度路径走 tensor core（mma.sync flash kernel，维度 32/64/128；2.5 的逐 lane 并行 softmax 再提升 1.6 倍）——为 SDPA bf16 flash 的 0.68-0.91x（tensor core 之前仅 0.11x）；f32 保持便捷路径（约为 SDPA 的 0.45x） |
...
cp311-cp313）与 **Windows x86_64**（cp311-cp313）。其他平台或 Python 版本
pip 会自动从源码构建：
...
kernel 通过 `data_ptr()` 直接读写 torch 显存缓冲区，无 staged 中转、无主机端同步。
...
# staged 路径：拷入 GPU、跑 kernel、结果拷回
...
返回同族的 float32 输出。CUDA torch 张量会自动选择零拷贝路径；所有搬运数据的算子（elementwise / 归一化 / RoPE / attention）在 CUDA 张量上支持
...
- 每个算子均附带 **CPU 参考实现** 与逐元素对照测试（parity，pytest）
- 无 GPU 的机器也能运行测试（CUDA 用例自动跳过）

## API 稳定性

1.0 冻结了当时的 34 个公开算子与辅助函数；此后新增算子均走小版本
发布，目前 `fusedtok.__all__` 共 57 个，1.x 系列内签名保持不变。
...
| kv_append（连续 cache 写入） | B=8, T=4096 | 16 µs | 49 µs（高级索引） | **3.06x** |
| sample_topp p=0.9（峰值分布） | [131072] | 146 µs | 390 µs（排序+掩码+multinomial） | **2.68x** |
| RMSNorm（含残差） | [4096×4096] | 606 µs | 2033 µs | **3.36x** |
...
| sample_minp p=0.05（峰值分布） | [131072] | 139 µs | 289 µs（掩码+multinomial） | **2.07x** |
| sample_topa a=0.2（峰值分布） | [131072] | 184 µs | 210 µs（掩码+multinomial） | **1.15x** |
| top-k（k=4096，中段 k） | [131072] | 72 µs | 136 µs | 1.89x（落败） |
| Softmax | [4096×4096] | 406 µs | 425 µs | **1.05x** |
| SiLU / GeLU / add | [4096×4096] | ~405-601 µs | ~406-601 µs | ~1.0x |
| argmax | [131072] | 32 µs | 34 µs | 1.08x（事件计时在 WDDM 上抖动大；3060 随库发布的三轮计时在 0.72-1.26x 间摆动，见下） |
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 3494 µs（38.6 TOPS） | 2023 µs（cuBLASLt + 广播） | 0.58x（落败） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 9373 µs（31.1 TOPS） | 4441 µs（cuBLASLt） | 0.47x（落败） |
| attention_prefill（因果） | S=1024, D=128 | 5701 µs | 2560 µs（SDPA flash） | 0.45x（落败） |
| attention_prefill bf16（v2.4；2.5 softmax） | S=1024, D=128 | 695 µs | 631 µs（SDPA bf16 flash） | 0.91x（落败；5060 Ti 为 0.68x） |
| sample_minp p=0.05（宽核） | [131072] | 508 µs | 286 µs | 0.56x（落败：一次扩窗重试加一次 32-64k 排序；torch 的布尔掩码组合式不用排序——min-p 的胜出场景见上方峰值行） |
| sample_nsigma 1.5（峰值分布） | [131072] | 2145 µs | 500 µs（掩码+multinomial） | 0.23x（落败：σ 阈值会保留尖峰行约前 94% 的词表——nsigma 剪的是分布的低处长尾，尖峰行因此接近全词表采样、走完整扩窗阶梯；论文主张 nsigma 的收益在输出质量而非速度） |
| sample_topp p=0.9（平坦最坏） | [131072] | 1314 µs | 393 µs | 0.30x（落败，见下） |
...
| sample_minp_batched p=0.05 | [8×131072] | 233 µs | 328 µs（掩码+multinomial） | **1.41x** |
| sample_topa_batched a=0.2 | [8×131072] | 287 µs | 297 µs（掩码+multinomial） | **1.03x** |
| sample_topp_batched p=0.9 | [8×131072] | 333 µs | 215 µs（multinomial） | 0.64x（参考侧 WDDM 波动，见下） |
| decode_step_batched（惩罚 1.3，~64 token 历史） | [8×131072] | 310 µs | 270 µs（惩罚+softmax+multinomial） | 0.87x（对比逐行循环墙上时钟 **5.2x**，见下方说明） |
| argmax_batched | [8×131072] | 33 µs | 24 µs（argmax） | 0.73x（对 torch 单 kernel 的逐行 argmax、事件计时），但对比逐行循环墙上时钟 **28x**（8 次提交 -> 1 次） |
| sample_nsigma_batched 1.5 | [8×131072] | 3771 µs | 322 µs（掩码+multinomial） | 0.09x（落败，同单行宽核说明） |
| sample_topp_batched（平坦最坏） | [8×131072] | 3625 µs | 233 µs | 0.06x（落败，同单行说明） |
...
| sample_topp p=0.9（峰值分布） | [131072] | 62 µs | 156 µs（排序+掩码+multinomial） | **2.51x** |
| sample_topk k=50 | [131072] | 45 µs | 93 µs（topk+multinomial） | **2.06x** |
| kv_append（连续 cache 写入） | B=8, T=4096 | 9 µs | 21 µs（高级索引） | **2.22x** |
...
| argmax | [131072] | 17 µs | 14 µs | 0.81x（事件计时有噪声） |
| int8 qgemm pc（W8A8） | [4096×4096×4096] | 2078 µs（66.1 TOPS） | 1142 µs（cuBLASLt + 广播） | 0.55x（落败） |
| attention_prefill（因果） | S=1024, D=128 | 3300 µs | 1421 µs（SDPA flash） | 0.43x（落败） |
| attention_prefill bf16（v2.4；2.5 softmax） | S=1024, D=128 | 388 µs | 263 µs（SDPA bf16 flash） | 0.68x（落败） |
| int8 qgemm（IMMA） | [4096×11008×4096] | 5483 µs（67.4 TOPS） | 2185 µs（cuBLASLt） | 0.40x（落败） |
| sample_minp p=0.05（宽核） | [131072] | 218 µs | 73 µs | 0.33x（落败：同 3060 行的宽核说明） |
| sample_nsigma 1.5（峰值分布） | [131072] | 1048 µs | 85 µs（掩码+multinomial） | 0.08x（落败：同 3060 行的长尾过滤器说明——尖峰行接近全词表采样） |
| sample_topp p=0.9（平坦最坏） | [131072] | 1054 µs | 167 µs | 0.16x（落败，见下） |
...
| sample_minp_batched p=0.05 | [8×131072] | 121 µs | 112 µs（掩码+multinomial） | 0.93x（持平） |
| sample_topa_batched a=0.2 | [8×131072] | 133 µs | 113 µs（掩码+multinomial） | 0.85x |
| decode_step_batched（惩罚 1.3，~64 token 历史） | [8×131072] | 147 µs | 99 µs（惩罚+softmax+multinomial） | 0.68x（对比逐行循环墙上时钟 **4.5x**，见下方说明） |
...
| sample_nsigma_batched 1.5 | [8×131072] | 1727 µs | 122 µs（掩码+multinomial） | 0.07x（落败，同单行长尾说明） |
| sample_topp_batched（平坦最坏） | [8×131072] | 1744 µs | 83 µs | 0.05x（落败） |

小形状下 Blackwell 的优势更大（softmax 1.72x、RMSNorm 3.08x @256 行、
...
胜过 eager 组合式（峰值行的组合参考在 WDDM 上逐轮波动明显，逐轮数值在 JSON）；平坦分布下 sample_topp 确实更慢 —— 此时核覆盖
约九成词表，管线实际上要给全词表排序。v1.2 用三处不改变任何输出 token 的改动（逐位不变）
把该最坏情况耗时压到约 1/8.5（3060 上 n=131072 实测 18.2ms ->
2.2ms）：按 p×总质量 下界自适应扩窗跳变；全词表快路径（窗口等于词表时
跳过选择阶段）；串行遍历改为批量取数——承担契约的严格顺序浮点加法
一步不动（它正是与 CPU 参考逐位一致的确定性契约本身），只有数据载入被
...
sample_minp（v1.3）在尖峰 logits 上胜出，宽核行确实更慢（一次扩窗
...
0.64x/0.62x、1.75x/1.19x、1.03x/0.85x——top-k 明确胜出，其余持平或
小输），平坦最坏情况比单行版还要再慢一档，这个差距也同样如实列在表中。v1.5 把批处理扩展到整个解码步：
...
SDPA bf16 flash 的 0.68-0.91x（见下方表格行）。
...
约 2.1-2.7 倍领先（W8A8 行的差距只有约 1.6-2.2 倍）：它的 tile 流水线更深、
...
上摆动大（3060 随库发布的三轮计时在 0.72-1.26x 间摆动）；把同步排除在计时循环之外的墙钟实测为
...
- 1.6.1（已发布）：`logit_penalties`——HF 三件惩罚（CTRL 重复缩放、出现惩罚位移、按计数加权的频率位移）一次调用全部完成，每个去重 id 只罚一次、重复不叠加，CPU 与 GPU 逐位一致——外加审计驱动加固：修复 `typical = 1.0` 时误报 "typical nucleus not covered"、批量 typical 缺失的信任边界、eta 扩窗下界的量纲错配（公开名称 42 -> 43）
...
- 2.4.4（已发布）：补上 [2.4.3] 声明了但未随版发布的 sample_xtc staged 绑定委托；W8A8 领先区间修正为覆盖自身表格（1.6-2.2 倍）
- 2.5（已发布）：sample_mirostat——带调用方 mu 界限的 Mirostat v2 熵目标采样（唯一返回元组的采样器：(token, new_mu)；公开名称 55 -> 57）；tensor-core prefill 的逐 lane 并行 softmax（探针 1.63 倍，bf16 行 0.58x -> 0.91x SDPA）；cp.async 暂存与 16-bit radix key 经评估诚实撤销
